...
export const aiPromptingTasks: PortfolioTask[] = [
  ...
  // --- Topic 12: Prompting for Vector Databases & Semantic Search ---
  {
    id: 'prompt-vector-metadata-extraction',
    categoryId: 'ai-prompting',
    topicId: 'prompting-vector-databases',
    level: 'beginner',
    title: 'Vector Database Metadata Extraction Prompt',
    difficulty: 'Beginner',
    estimatedHours: '2 hours',
    tagline: 'Extract key metadata fields from raw text as structured JSON for vector search.',
    scenario:
    ...
    rubric: [
      { key: 'schema_accuracy', label: 'Schema Accuracy', description: 'Does the prompt consistently output valid JSON conforming to the defined metadata structure?', weight: 40 },
      { key: 'cleaning_sanitization', label: 'Cleaning & Sanitization', description: 'Are variables (like currency, location) normalized properly (e.g. converting ZAR or EUR to USD)?', weight: 30 },
      { key: 'edge_cases', label: 'Edge Case Handling', description: 'Does the prompt gracefully handle missing or ambiguous fields (e.g. "several years of experience")?', weight: 30 }
    ],
    skillsProven: ['Metadata extraction', 'Structured JSON output', 'Data normalization', 'Vector hybrid search design'],
    relatedRoadmapIds: []
  },
  {
    id: 'prompt-vector-query-rewrite',
    categoryId: 'ai-prompting',
    topicId: 'prompting-vector-databases',
    level: 'intermediate',
    title: 'Semantic Query Rewriting for Search Optimization',
    difficulty: 'Intermediate',
    estimatedHours: '3 hours',
    tagline: 'Design a query rewriting prompt that maps conversational searches to search terms and filters.',
    scenario:
    ...
    rubric: [
      { key: 'query_rewrite_logic', label: 'Query Rewrite Logic', description: 'Does the prompt correctly separate semantic intent from hard constraints?', weight: 35 },
      { key: 'metadata_classification', label: 'Filter Classification', description: 'Are filter parameters (price thresholds in USD, category tags) parsed and mapped accurately?', weight: 35 },
      { key: 'latency_efficiency', label: 'Latency & Token Optimization', description: 'Are token savings and caching guidelines correctly addressed in prompt design?', weight: 30 }
    ],
    skillsProven: ['Query rewriting', 'Metadata filtering', 'Semantic query expansion', 'Vector database search optimization'],
    relatedRoadmapIds: []
  },
  {
    id: 'prompt-vector-hybrid-retrieval',
    categoryId: 'ai-prompting',
    topicId: 'prompting-vector-databases',
    level: 'advanced',
    title: 'Hybrid Retrieval Prompt Orchestration Framework',
    difficulty: 'Advanced',
    estimatedHours: '5 hours',
    tagline: 'Build an orchestration prompt chain that filters, reranks, and ground-answers search results.',
    scenario:
    ...
    rubric: [
      { key: 'hallucination_mitigation', label: 'Hallucination Mitigation', description: 'Does the system prompt enforce strict grounding, and prevent answering when facts are missing?', weight: 35 },
      { key: 'prompt_chaining_logic', label: 'Prompt Chain Orchestration', description: 'Is the sequence of filtering, citation-mapping, and answering logical and cohesive?', weight: 35 },
      { key: 'token_cost_optimization', label: 'Token & Cost Management', description: 'Does the architecture present viable strategies for managing token cost and window limitations?', weight: 30 }
    ],
    skillsProven: ['RAG architecture design', 'Hallucination mitigation', 'Source citation prompting', 'Context engineering', 'Token optimization'],
    relatedRoadmapIds: []
  },

  // --- Topic 13: Prompt Security & Jailbreak Mitigation ---
  {
    id: 'prompt-jailbreak-assessment',
    categoryId: 'ai-prompting',
    topicId: 'prompt-security-jailbreak',
    level: 'beginner',
    title: 'Basic Prompt Vulnerability Assessment & Patching',
    difficulty: 'Beginner',
    estimatedHours: '2 hours',
    tagline: 'Audit a customer service prompt for security weaknesses and write defensive rules.',
    scenario:
    ...
    rubric: [
      { key: 'vulnerability_mapping', label: 'Vulnerability Analysis', description: 'Are the three vulnerability vectors correctly identified and explained?', weight: 35 },
      { key: 'defense_patching', label: 'Defense Implementation', description: 'Does the patched prompt successfully prevent basic instruction overrides without breaking normal functionality?', weight: 35 },
      { key: 'testing_verification', label: 'Testing Scenarios', description: 'Are the test jailbreak payloads diverse and do they accurately demonstrate defense effectiveness?', weight: 30 }
    ],
    skillsProven: ['Vulnerability assessment', 'Prompt safety design', 'Jailbreak mitigation', 'Adversarial prompt testing'],
    relatedRoadmapIds: []
  },
  {
    id: 'prompt-defense-sandbox',
    categoryId: 'ai-prompting',
    topicId: 'prompt-security-jailbreak',
    level: 'intermediate',
    title: 'Input Pre-Filter and Guardrail System Prompt',
    difficulty: 'Intermediate',
    estimatedHours: '3 hours',
    tagline: 'Design an application firewall prompt that screens and sanitizes user inputs before the main LLM step.',
    scenario:
    ...
    rubric: [
      { key: 'filter_accuracy', label: 'Filter Classification', description: 'Does the prompt specify clear and distinct risk categories and evaluation logic?', weight: 35 },
      { key: 'payload_detection', label: 'Security JSON Output', description: 'Is the JSON output schema robust, parseable, and includes all necessary security parameters?', weight: 35 },
      { key: 'false_positive_mitigation', label: 'False Positive Mitigation', description: 'Does the solution provide a sensible strategy to handle false positives in client communications?', weight: 30 }
    ],
    skillsProven: ['API firewall design', 'Safety filter prompting', 'Adversarial risk classification', 'Structured JSON security logs', 'Dual-LLM architecture'],
    relatedRoadmapIds: []
  },
  {
    id: 'prompt-red-teaming-audit',
    categoryId: 'ai-prompting',
    topicId: 'prompt-security-jailbreak',
    level: 'advanced',
    title: 'Enterprise Red-Teaming & Layered Defense System Prompt',
    difficulty: 'Advanced',
    estimatedHours: '5 hours',
    tagline: 'Perform a comprehensive red-teaming audit and design a production-grade layered defense system prompt.',
    scenario:
    ...
    rubric: [
      { key: 'threat_modeling', label: 'Threat Modeling & Exploit Paths', description: 'Are the advanced exploit vectors explained clearly with realistic, sophisticated examples?', weight: 35 },
      { key: 'layered_defense_design', label: 'Layered Defense Architecture', description: 'Does the system prompt utilize advanced techniques (like XML schemas, input wrapping, instructions priority) effectively?', weight: 35 },
      { key: 'policy_safeguards', label: 'Continuous Monitoring & Policy', description: 'Are the security policies and continuous monitoring suggestions practical, scalable, and secure?', weight: 30 }
    ],
    skillsProven: ['Red-teaming', 'Threat modeling', 'XML delimiters and data isolation', 'Layered prompt defense', 'AI security policy design'],
    relatedRoadmapIds: []
  }
];
